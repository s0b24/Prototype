...
class DatabaseHelper(private val context: Context) : SQLiteOpenHelper(context, "components.db", null, 1) {
    ...
    fun getAveragePriceByCountry(componentId: Int, countryCode: String): Double? {
        val db = readableDatabase
        val cursor = db.rawQuery(
            """
                SELECT AVG(price)
            ...
            """, arrayOf(componentId.toString(), countryCode)
        )
    ...
    fun getComponentCard(type: String, countryCode: String): List<ComponentCard> {
        val result = mutableListOf<ComponentCard>()
        val components = getComponentByType(type)

        for (c in components) {
            val specifications = getComponentSpecs(c.id)
            val price = getAveragePriceByCountry(c.id, countryCode)
    ...
    fun getStorePrices(componentId: Int, countryCode: String): List<StorePriceEntity> {
    ...
    fun saveConfiguration(configurationId: Int, selected: Map<String, ComponentSpecs>, totalPrice: Double) {
        ...
        val name = "Configuration $nextNum"
    ...
    fun getMaxCpuSpecsValues(): Map<String, Double> {
        val db = readableDatabase
        val result = mutableMapOf<String, Double>()
        val keys = listOf(
            SpecKeys.CPU_Cores,
            SpecKeys.CPU_Threads,
            SpecKeys.CPU_Boost_Clock
        )

        for (key in keys) {
            val cursor = db.rawQuery(
                """
                    SELECT MAX(CAST(s.spec_value AS REAL))
                    FROM SPECIFICATIONS s
                    JOIN COMPONENTS c ON s.component_id = c.id
                    WHERE c.type = ? AND s.spec_key = ?
                """, arrayOf("cpu", key)
            )

            if (cursor.moveToFirst()) {
                result[key] = cursor.getDouble(0)
            }
            cursor.close()
        }
        return result
    ...
    fun getMaxGpuSpecsValues(): Map<String, Double> {
        val db = readableDatabase
        val result = mutableMapOf<String, Double>()
        val keys = listOf(
            SpecKeys.GPU_Clock,
            SpecKeys.GPU_Memory_Capacity,
            SpecKeys.GPU_Memory_Speed,
            SpecKeys.GPU_Memory_Bus
        )
    ...
    fun getMaxRamSpecsValues(): Map<String, Double> {
        val db = readableDatabase
        val result = mutableMapOf<String, Double>()
        val keys = listOf(
            SpecKeys.Ram_Capacity,
            SpecKeys.Ram_Frequency,
            SpecKeys.Ram_CL
        )
